...
import com.example.demo.activity.dao.ActMapper;
import com.example.demo.activity.entity.Act;
import com.example.demo.activity.entity.RocIdUser;
import com.example.demo.activity.util.ActUtils;
...
public class ActTaskService {
	 ...
	 * @return
	 */
	public PageResult  todoList(int pageNum,int pageSize, Act act,String userId){
...
//		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		...
		PageInfo<Task> info = new PageInfo<>(todoTaskQuery.list());
	 ...
	 * @return
	 */
	public PageResult historicList(int pageNum,int pageSize, Act act,String userId){
		PageResult r = new PageResult();
	//	String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		PageHelper.startPage(pageNum, pageSize);
		...
		PageInfo<HistoricTaskInstance> info = new PageInfo<>(histTaskQuery.list());
		// 查询总数
	//	page.setCount(histTaskQuery.count());
	//	page.initialize();
		
		List<Map> list = new ArrayList<>();
		for (HistoricTaskInstance histTask : info.getList()) {
			Act e = new Act();
...
//			map.put("procDef.version", ProcessDefCache.get(histTask.getProcessDefinitionId()).getVersion());
		...
		r.setTotal(histTaskQuery.count());r.setObject(list);
		return r;
	}
	 ...
	 */
	public List<Act> histoicFlowList(String procInsId, String startAct, String endAct,RocIdUser user){
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	...
	public PageResult processList(int pageNum,int pageSize, String category) {
	    ...
	    PageInfo<ProcessDefinition> info = new PageInfo<>(processDefinitionQuery.list());
	    
	    List<Object[]> list = new ArrayList<>();
	    for (ProcessDefinition processDefinition : info.getList()) {
...
//		logger.debug("getFormKey: {}", formKey);
	 ...
	 */
	@Transactional(readOnly = false)
	public List<ProcessInstance> getRunngingProcIns(String procDefKey,  RocIdUser user, int[] pageParams) {
	 ...
	 */
	@Transactional(readOnly = false)
	public List<HistoricProcessInstance> getFinishedProcIns(String procDefKey, RocIdUser user, int[] pageParams) {
	 ...
	 */
	@Transactional(readOnly = false)
	public PageResult  getMyStartedProcIns( RocIdUser user, int pageNum,int pageSize) throws  Exception{
		PageResult r = new PageResult();
		PageHelper.startPage(pageNum, pageSize);
		HistoricProcessInstanceQuery query = historyService.createHistoricProcessInstanceQuery().startedBy(user.getUserName()).includeProcessVariables().orderByProcessInstanceStartTime().desc();
		PageInfo<HistoricProcessInstance> info = new PageInfo<>(query.list());
...
//			historicProcessInstance.
		...
		r.setTotal(query.count());r.setObject(list);
		return r;
	}
	/**
	 * 启动流程
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, String businessId,RocIdUser user) {
		return startProcess(procDefKey, businessTable, businessId, "", user);
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, String businessId, String title,RocIdUser user) {
		Map<String, Object> vars = Maps.newHashMap();
		return startProcess(procDefKey, businessTable, businessId, title, vars,user);
	}
	...
	@Transactional(readOnly = false)
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars,RocIdUser user) {
		//String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId())
		// 设置流程变量
		if (vars == null){
	 ...
	 * @param procInsId
	 */
	public void completeFirstTask(String procInsId,String userId){
		completeFirstTask(procInsId, null, null, null,userId);
	}
